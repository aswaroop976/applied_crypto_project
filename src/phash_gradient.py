...
PHASH_THRESHOLD = 35 # Represents hamming distance between PDQ hashes, change later
...
def iterative_greedy_attack(orig_rgb, x_small, its=50, M=200, eps_step=0.005, eps_max=0.05, candidate_type='gauss', threshold=20):
    ...
    dist0, sim0 = phash_distance_gray(x_small, np.zeros_like(x_small))
    ...
    for t in range(its):
        ...
        # debug/log
        print(f"iter {t+1:3d}: best_candidate_dist={best_dist:3d} => cum_dist={dist_cum:3d} PDQsim={sim_cum:.3f}")
